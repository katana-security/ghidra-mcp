package eu.starsong.ghidra.endpoints;
    ...
    import eu.starsong.ghidra.util.StringCacheManager;
    ...
    public class DataEndpoints extends AbstractEndpoint {
        ...
        private final StringCacheManager stringCache = new StringCacheManager();
        ...
        public void registerEndpoints(HttpServer server) {
            ...
            // Register cache endpoints BEFORE /strings (HttpServer matches longest prefix)
            server.createContext("/strings/cache/status", exchange -> {
                try {
                    if ("GET".equals(exchange.getRequestMethod())) {
                        handleStringsCacheStatus(exchange);
                    } else {
                        sendErrorResponse(exchange, 405, "Method Not Allowed");
                    }
                } catch (Exception e) {
                    Msg.error(this, "Error in /strings/cache/status endpoint", e);
                    sendErrorResponse(exchange, 500, "Internal server error: " + e.getMessage());
                }
            });
            server.createContext("/strings/cache/invalidate", exchange -> {
                try {
                    if ("POST".equals(exchange.getRequestMethod())) {
                        handleStringsCacheInvalidate(exchange);
                    } else {
                        sendErrorResponse(exchange, 405, "Method Not Allowed");
                    }
                } catch (Exception e) {
                    Msg.error(this, "Error in /strings/cache/invalidate endpoint", e);
                    sendErrorResponse(exchange, 500, "Internal server error: " + e.getMessage());
                }
            });
         ...
         * Uses StringCacheManager for O(1) pagination on cached data.
         */
        public void handleListStrings(HttpExchange exchange) throws IOException {
            try {
                ...
                String filterLower = (filter != null && !filter.isEmpty()) ? filter.toLowerCase() : null;
                ...
                // Get or build cached string list (thread-safe, auto-invalidates on program change)
                List<StringInfo> allStrings = stringCache.getOrBuild(program);

                // Apply filter if present
                List<StringInfo> matched;
                if (filterLower != null) {
                    matched = new ArrayList<>();
                    for (StringInfo si : allStrings) {
                        if (si.getValueLower().contains(filterLower)) {
                            matched.add(si);
                        }
                    }
                } else {
                ...
                // O(1) pagination via subList
                int start = Math.min(offset, totalMatched);
                int end = Math.min(start + limit, totalMatched);
                List<StringInfo> page = matched.subList(start, end);

                // Convert to response maps
                List<Map<String, Object>> strings = new ArrayList<>(page.size());
                for (StringInfo si : page) {
                    Map<String, Object> item = new HashMap<>();
                    item.put("address", si.getAddress());
                    item.put("value", si.getValue());
                    item.put("length", si.getLength());
                    item.put("type", si.getTypeName());
                    item.put("name", si.getSymbolName());

                    Map<String, Object> links = new HashMap<>();
                    Map<String, String> selfLink = new HashMap<>();
                    selfLink.put("href", "/data/" + si.getAddress());
                    links.put("self", selfLink);

                    Map<String, String> memoryLink = new HashMap<>();
                    memoryLink.put("href", "/memory?address=" + si.getAddress());
                ...
                builder.result(strings);

                // Pagination metadata
                Map<String, Object> metadata = new HashMap<>();
                metadata.put("size", totalMatched);
                ...
                if (offset + limit < totalMatched) {
                    builder.addLink("next", "/strings?offset=" + (offset + limit) + "&limit=" + limit +
                                   (filter != null ? "&filter=" + filter : ""));
                }
                if (offset > 0) {
                    int prevOffset = Math.max(0, offset - limit);
                    builder.addLink("prev", "/strings?offset=" + prevOffset + "&limit=" + limit +
                                   (filter != null ? "&filter=" + filter : ""));
                }
        ...
        /**
         * Handle GET /strings/cache/status - returns cache debug info.
         */
        private void handleStringsCacheStatus(HttpExchange exchange) throws IOException {
            try {
                Program program = getCurrentProgram();
                StringCacheManager.CacheStatus status = stringCache.getStatus(program);

                Map<String, Object> result = new HashMap<>();
                result.put("cached", status.cached);
                result.put("size", status.size);
                result.put("buildTimeMs", status.buildTimeMs);
                result.put("valid", status.valid);
                result.put("modificationNumber", status.modificationNumber);

                eu.starsong.ghidra.api.ResponseBuilder builder = new eu.starsong.ghidra.api.ResponseBuilder(exchange, port)
                    .success(true)
                    .result(result);
                builder.addLink("strings", "/strings");
                builder.addLink("invalidate", "/strings/cache/invalidate");

                sendJsonResponse(exchange, builder.build(), 200);
            } catch (Exception e) {
                Msg.error(this, "Error getting string cache status", e);
                sendErrorResponse(exchange, 500, "Error getting cache status: " + e.getMessage(), "INTERNAL_ERROR");
            }
        }

        /**
         * Handle POST /strings/cache/invalidate - forces cache rebuild on next request.
         */
        private void handleStringsCacheInvalidate(HttpExchange exchange) throws IOException {
            try {
                stringCache.invalidate();

                Map<String, Object> result = new HashMap<>();
                result.put("message", "String cache invalidated");

                eu.starsong.ghidra.api.ResponseBuilder builder = new eu.starsong.ghidra.api.ResponseBuilder(exchange, port)
                    .success(true)
                    .result(result);
                builder.addLink("strings", "/strings");
                builder.addLink("status", "/strings/cache/status");

                sendJsonResponse(exchange, builder.build(), 200);
            } catch (Exception e) {
                Msg.error(this, "Error invalidating string cache", e);
                sendErrorResponse(exchange, 500, "Error invalidating cache: " + e.getMessage(), "INTERNAL_ERROR");
            }
        }
    }
